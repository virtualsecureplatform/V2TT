...
int main(int argc, char *argi[]) {
    ...
    LweSample* ciphertext = new_gate_bootstrapping_ciphertext_array(8600, params);
    //Unused ones
    ...
    //Initialize Buffer
    for(int i = 0; i <8600 - 4098;i++)bootsSymEncrypt(&ciphertext[i + 4098], 0, key);

    //export the 8600 ciphertexts to a file (for the cloud)
    FILE* cloud_data = fopen("cloud.data","wb");
    for (int i=0; i<8600; i++) export_gate_bootstrapping_ciphertext_toFile(cloud_data, &ciphertext[i], params);
    fclose(cloud_data);

    //clean up all pointers
    delete_gate_bootstrapping_ciphertext_array(8600, ciphertext);
